...
class PerceptualLoss(nn.Module):
    ...
    def __init__(self, net="vgg", log_flag=True):
        super().__init__()
    ...
    def forward(self, spec_sample, spec_target): # [B, 1, F, T] complex
        spec_sample = spec_sample.abs()
        spec_target = spec_target.abs()
        if self.log_flag:
            spec_sample = torch.log(spec_sample + 1e-5)
            spec_target = torch.log(spec_target + 1e-5)
            
        spec_sample_input = torch.concat([spec_sample,spec_sample,spec_sample], dim=1) # [B, 3, F, T] complex
        spec_target_input = torch.concat([spec_target, spec_target, spec_target], dim=1) # [B, 3, F, T] complex
        loss = self.loss(spec_sample_input, spec_target_input)
