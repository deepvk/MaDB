...
class DownSample(nn.Module):
    def __init__(
        self, input_channel, out_channel, kernel_size, stride, activation, normalization
    ):
        super().__init__()
        self.conv = nn.Sequential(
            CausalConv2d(
                input_channel, out_channel, kernel_size=kernel_size, stride=stride
            ),
            normalization,
            activation,
        )
...
class Unet_model(nn.Module):
    ...
    def forward(self, x, h_t=None):
        ...
        x = x6.view(B, -1, T).permute(
            0, 2, 1
        )  # (B, C, F, T) -> (B, C*F, T) -> (B, T, C*F): [B, T, 640]
        x, h_t = self.lstm(x, h_t)
        ...
        return self.convT(x), h_t  # -> [B, 1, 200, T]
